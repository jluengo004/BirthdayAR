...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        guard let trackingObjects = ARReferenceObject.referenceObjects(inGroupNamed: "soraa", bundle: nil) else{
            fatalError("Couldn't load tracking images")
        }
...
//        guard let name = imageAnchor.referenceImage.name else { return nil }
        guard let imageAnchor = anchor as? ARObjectAnchor else { return nil }
        guard let name = imageAnchor.referenceObject.name else { return nil }
